...
function getDefaultSymbolGroups(): string[][] {
  const env = getCollectorEnv();
  return chunk(splitCsv(env.COLLECTOR_DEFAULT_SYMBOLS), SYMBOL_GROUP_SIZE);
}

async function runGroupedDefaultCollection() {
  const env = getCollectorEnv();
  const symbols = splitCsv(env.COLLECTOR_DEFAULT_SYMBOLS);
  const symbolGroups = getDefaultSymbolGroups();
...
export async function handleCronRequest(
  ...
  try {
    if (typeof options?.groupIndex === "number") {
      const symbolGroup = getDefaultSymbolGroups()[options.groupIndex];
      if (!symbolGroup) {
        return NextResponse.json({ ok: false, error: "Unknown symbol group" }, { status: 404 });
      }

      const result = await runCollection({ symbols: symbolGroup });
      return NextResponse.json({ ok: true, mode: "group", group_index: options.groupIndex, ...result });
    }

    if (requestedSymbols.length === 0) {
      return await runGroupedDefaultCollection();
    }
